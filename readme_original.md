...
Данный проект представляет собой рефакторинг монолитной архитектуры в микросервисную с использованием современных технологий и подходов.

### Выполненные изменения:

1. **Разделение монолита на микросервисы**:
   - API Gateway (порт 8000) - единая точка входа с Circuit Breaker
   - Service Users (порт 3001) - управление пользователями
   - Service Orders (порт 3002) - управление заказами
   - Service Warehouse (порт 3003) - управление складом (вариант 4)

2. **Замена хранения в памяти на PostgreSQL**:
...
3. **Добавление кэширования Redis**:
   - Cache-aside стратегия для GET запросов по ID
   - Инвалидация кэша при создании/обновлении/удалении
   - TTL 5 минут для кэшированных данных

4. **Реализация дополнительного сервиса управления складом** (вариант 4):
   - Управление остатками товаров
   - Интеграция с сервисом заказов
   - Автоматическое уменьшение количества при создании заказа
